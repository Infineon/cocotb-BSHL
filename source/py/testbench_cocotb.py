...
#  Author      : See AUTHORS
#  Project     : cocotb-BSHL
#  Description : testbench with cocotb-coverage
...
class ALUCoverage():
    range_relation = lambda operand_a, bin_: bin_[0] <= operand_a <= bin_[1]
    # to collect data of operand_a in two bins: low and high

    @CoverPoint(
            name="top.operation", 
            bins = list(Ops),
            bins_labels = [ "ADD","SUB", "NOT", "NOR", "NAND", "AND", "OR", "XOR"],
            at_least = 1, 
            xf = lambda operand_a,operand_b,operation,transition: operation 
            #lambda func is used to extract values to be covered
    )

    @CoverPoint(
            name="top.operand_a", 
            rel = range_relation,
            bins = [(0,int(2**DATA_WIDTH/2)),(int(2**DATA_WIDTH/2),2**DATA_WIDTH)], 
            bins_labels = ["low", "high"],
            xf = lambda operand_a,operand_b,operation,transition: operand_a
    )

    @CoverPoint(
            name="top.operand_b",
            bins = list(range(2**DATA_WIDTH)),  # auto bins generated
            at_least = 1,
            weight = 2,
            xf = lambda operand_a,operand_b,operation,transition: operand_b
    )

    @CoverPoint(
            name="top.op_transition",
            vname="transition",
            bins= [(p,c) for p in list(Ops) for c in list(Ops)],
            xf = lambda operand_a,operand_b,operation,transition: transition
            # transition coverage of operands
    )

    @CoverCross(
            name="top.operands_cross", 
            items = ["top.operand_a", "top.operand_b"],
            ign_bins = ignore_bins #previously defined  
    )

    def sample_alu_coverage(self,operand_a,operand_b, operation, transition): 
        # when the func is called, parameters defined in the decorators will be automatically sampled
        pass
    
    def transition_relation(self):
        if len(operation_covered)>1:
            prev_op, curr_op = Ops(operation_covered[-2]), Ops(operation_covered[-1])
...
class Coverage(uvm_subscriber):
    ...
    def write(self, cmd):
        ...
        transition = alu_cov.transition_relation()
        
        #illegal bins and conditional coverage
        
        if b in illegal_bins:
            # print(f"Illegal condition detected: operand_b is {b}")
            self.logger.info(f"Illegal condition detected: operand_b is {b}")
        
        if (a,b) not in operands_covered and b not in illegal_bins:# conditional sampling
            alu_cov.sample_alu_coverage( a, b, op, transition)
            operands_covered.append((a,b)) # record combinations of operand_a and operand_b which are already covered
            operation_covered.append(op) # record operations to track the transition coverage of operations
        self.logger.info(f"Ignore bins: {ignore_bins}")
    ...
    def report_phase(self):
        ...
        if not disable_errors:
            ...
            coverage_db.export_to_xml(filename="coverage_result_cocotb.xml")
            assert True
...
class Scoreboard(uvm_component):
    ...
    def check_phase(self):
        ...
        while self.result_get_port.can_get():
            ...
            _, self.actual_result = self.result_get_port.try_get()
            cmd_success, cmd = self.cmd_get_port.try_get()
            if not cmd_success:
                self.logger.critical(f"result {self.actual_result} had no command")
            else:
                (A, B, op_numb) = cmd
                op = Ops(op_numb)
                predicted_result = alu_prediction(A, B, op, self.errors)
                if predicted_result == self.actual_result:
                    # self.logger.info(f"At {sequence_sim_time} ns, PASSED: 0x{A:0{DATA_WIDTH}x} {op.name} 0x{B:0{DATA_WIDTH}x} ="
                    #                  f" 0x{self.actual_result:0{DATA_WIDTH}x}")
                    pass
                else:
                    self.logger.error(f"FAILED: 0x{A:0{DATA_WIDTH}x} {op.name} 0x{B:0{DATA_WIDTH}x} "
                                      f"= 0x{self.actual_result:0{DATA_WIDTH}x} "
                                      f"expected 0x{predicted_result:0{DATA_WIDTH}x}")
                    passed = False
        assert passed
...
class AluTest(uvm_test):
    ...
    async def run_phase(self):
        ...
        cg_operation = coverage_db["top.operation"]
        print(f"cover_percentage of top.operation:{cg_operation.cover_percentage}")
        cg_operation.add_threshold_callback(lambda: print(f"exceeded {50}%"),50)
        self.drop_objection()
